...
class InstructorTrainer(Seq2SeqTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        for i in range(0, num):
            ...
            cur_score = similarity_fct(anchor_emb, pos_emb) / self.args.cl_temperature
            # GG_NOTE: Changes from above are
            # anchor_emb:  embeddings_query --> embeddings_pos
            # pos_emb:  embeddings_pos --> embeddings_query
            # one_neg_emb:  embeddings_neg --> embeddings_query
            for j in range(0, num):
                if i == j:
                    continue
...
def main():
    ...
    with open(os.path.join(model_args.cache_dir, "medi-data/medi-data.json")) as f:
    ...
    print(train_raw_datasets["train"][0])
